...
# ===== CONFIG =====
CATEGORIES_FILE = r"D:\VSCode\DA3\tiki_leaf_categories.csv"
OUTPUT_FILE = r"D:\VSCode\DA3\tiki_dataset.jsonl"  # NDJSON
MAX_WORKERS = 2   # giảm để tránh bị chặn
MAX_PAGES = 20    # giới hạn 20 trang mỗi category
    ...
    for attempt in range(retries):
        try:
            ...
            if r.is_redirect or r.status_code in (301, 302, 303, 307, 308):
                print(f"⚠️ Redirect at {url}")
                return {"error": "redirect"}

            if r.status_code == 429:
                print(f"⚠️ Rate limited (429). Sleeping before retry... {url}")
                time.sleep(3)
                continue
